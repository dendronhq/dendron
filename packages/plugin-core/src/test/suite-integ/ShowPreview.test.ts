...
import { before, test, beforeEach } from "mocha";
...
suite("GIVEN ShowPreview", function () {
  ...
  beforeEach(() => {
    cmd = new TogglePreviewCommand(
      ...
      // By setting isShowPreview to true, TogglePreviewCommand is used for the Show Preview command
      true
    );
  });
  ...
  describeSingleWS("WHEN preview panel is already open", {}, () => {
    ...
    test("THEN the preview should NOT be hidden", async () => {

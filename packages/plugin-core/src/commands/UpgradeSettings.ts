...
import { Extensions, Settings, SettingsUpgradeOpts } from "../settings";
...
export class UpgradeSettingsCommand extends BaseCommand<UpgradeSettingsCommandOpts> {
  async execute(opts: UpgradeSettingsCommandOpts) {
    const ctx = "execute";
    L.info({ ctx, opts });
    const config = opts.ws.configWS;
    if (!config) {
    ...
    Extensions.update(path.dirname(opts.ws.workspaceFile.fsPath));
    return;
  }
}

...
import { ExternalService, PodExportScope, PodV2Types } from "@dendronhq/pods-core";
      ...
      test("THEN podArgs are enriched", async () => {
        await runEngineTestV5(
          async ({ wsRoot }) => {
            ...
            expect(resp.data?.config.podType).toEqual(PodV2Types.MarkdownExportV2);
            expect(resp.data?.config.exportScope).toEqual(PodExportScope.Workspace);
            return;
          },
          { expect }
        );
    ...
    test("THEN podArgs are enriched", async () => {
      await runEngineTestV5(
        async ({ wsRoot }) => {
          ...
          const resp = await cmd.enrichArgs({
            wsRoot,
            inlineConfig: ["Key=podType,Value=MarkdownExportV2", "Key=exportScope,Value=Note", "Key=destination,Value=clipboard"],
            fname: "root",
            vault: "vault1"
          });
          expect(resp.data?.config.podType).toEqual(PodV2Types.MarkdownExportV2);
      ...
      test("THEN pod args are enriched with service connection values", async () => {
        await runEngineTestV5(
          async ({ wsRoot }) => {
            ...
            expect(resp.data?.config.podType).toEqual(PodV2Types.GoogleDocsExportV2);
            return;
          },
          { expect }
        );

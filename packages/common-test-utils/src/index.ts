...
/**
 * Utility class to make the tests against vault to be concise while having descriptive
 * failure messages. */
export class VaultAsserter {
  private readonly workspaceVault: WorkspaceVault;

  constructor(workspaceVault: WorkspaceVault) {
    this.workspaceVault = workspaceVault;
  }

  /** Asserts that no match is found  */
  async noMatch(noMatchList: string[]) {
    const hasNoneMatched = await EngineTestUtilsV4.checkVault({
      ...this.workspaceVault,
      nomatch: noMatchList,
    });
    ...
    assert(
      hasNoneMatched,
      `Found unexpected match in '${JSON.stringify(
        noMatchList
      )}' while checking ${this.vaultToString()}`
    );
  }

  /** Asserts that match is found  */
  async match(matchList: string[]) {
    const hasFoundMatch = await EngineTestUtilsV4.checkVault({
      ...this.workspaceVault,
      match: matchList,
    });
    ...
    assert(
      hasFoundMatch,
      `Did not find match for '${JSON.stringify(
        matchList
      )}' while checking ${this.vaultToString()}`
    );
  }

  private vaultToString() {
    return `Vault fsPath:'${this.workspaceVault.vault.fsPath}' vaultName: '${this.workspaceVault.vault.name}'`;
  }
}

/** Asserter utility to have more concise tests, while having descriptive failure messages.
 *  Supports fluent API. */
export class NoteAsserter {
  private readonly workspaceVault: WorkspaceVault;
  private readonly notes: NoteProps[] | NotePropsDict;

  constructor(opts: {
    workspaceVault: WorkspaceVault;
    notes: NoteProps[] | NotePropsDict;
  }) {
    this.workspaceVault = opts.workspaceVault;

    if (!_.isArray(opts.notes)) {
      this.notes = _.values(opts.notes);
    } else {
      this.notes = opts.notes;
    }
  }

  noMatchByFName(fname: string): NoteAsserter {
    const hasNoMatch = _.isUndefined(
    ...
    assert(
      hasNoMatch,
      `Expected there to be no match for fname='${fname}' but found a match. While checking note asserter '${this.toString()}'`
    );

    return this;
  }

  matchByFName(fname: string): NoteAsserter {
    const hasMatch = NoteUtils.getNoteByFnameV5({
      fname: fname,
      notes: this.notes,
      ...this.workspaceVault,
    });
    ...
    assert(
      hasMatch,
      `Expected there be a match for fname='${fname}' but did not find a match. While checking note asserter '${this.toString()}'`
    );
  ...
  toString(): string {
    return `Vault name: '${this.workspaceVault.vault.name}', Vault path: '${this.workspaceVault.vault.fsPath}'`;
  }
}

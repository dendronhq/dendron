...
export type NodeStore = {
  query: (scope: Scope, regex: RegExp) => Promise<IdDict>;
...
export default class Engine {
  ...
  public async query(
    ...
    const ids = await this.storage.query(scope, combined_regex);
    const nodes = await this.storage.getBatch(
      scope,
      Object.values(ids),
      nodeType
    );
    ...
    return {
      item: this.fromStubs(this.update_paths(ids, regexes), nodes.item),
      error: null,
      ...nodes,
    };
  }
  ...
  private query_to_regexes(query: string): [string, string][] {
    ...
    for (const term of terms) {
      const [filter, replacement] = this.split_term(term);
      if (filter.indexOf("=") > 0) {
        // nonempty explicit match (/foo=bar/ and /foo=/)
        i = 1;
        if (filter.indexOf("=") == filter.length - 1) {
          // match the schema with any value
        ...
        // positional match (/*/, /**/, and /bar/)
        const group = replacement.length == 0 ? "" : "$" + i;
    ...
    if (!terms[0].includes("="))
      filter_groups[0][0][0] = "^" + filter_groups[0][0][0]; // start at top
  ...
  private update_paths(paths: IdDict, regexes: [string, string][]): IdDict {
    const regexps: [RegExp, string][] = regexes.map((r) => [
      RegExp(r[0]),
      r[1],
    ]);
    const out: IdDict = {};
    for (const path in paths) {
      const key = regexps
        .map((r) => (path.match(r[0]) || [""])[0].replace(r[0], r[1]))
        .join("/");
      out[key] = paths[path];
    }
    return out;
    ...
    const helper = (i: number, parent_path: string) => {
      while (i < paths.length && paths[i].startsWith(parent_path)) {
        ...
        out[ids[paths[i]]] = {
          ...
          children: (stubs[ids[paths[i]]] as DNode).children || [],
        };
        i++;
        if (i < paths.length && paths[i].startsWith(paths[i - 1])) {
          out[ids[paths[i - 1]]].children = [];
        } // clear implicit children if there are explicit ones
        helper(i, paths[i - 1]);
      }
    };
    out[ids[paths[0]]] = {
      ...stubs[ids[paths[0]]],
      parent: (stubs[ids[paths[0]]] as DNode).parent || null,
      children: [],
    };
    helper(1, paths[0]);
    return out;

import {
  ...
  EngineWriteRequest, GetNoteBlocksPayload, GetNoteBlocksRequest, NoteQueryRequest, RenderNoteOpts, WriteNoteResp
...
router.post("/getByPath", asyncHandler(async (req: Request, res: Response) => {
...
router.post("/write", asyncHandler(async (req: Request, res: Response<WriteNoteResp>) => {
...
router.get(
  ...
  asyncHandler(
    async (req: Request, res: Response<GetNoteBlocksPayload>) => {
        const { id, ws, filterByAnchorType } =
          req.query as GetNoteBlocksRequest;
        const engine = await getWSEngine({ ws: ws || "" });
        ExpressUtils.setResponse(res, await engine.getNoteBlocks({ id, filterByAnchorType }));
    }
  )
);
